<template>
  <ErpLayout title="ERP - Edit Trainings" :getInfo="() => getCollectionInfo(api)">
    <template #headline>
    ...
    <div class="erp-upload-form">
      <div class="erp-form-section-header erp-no-top-margin">
        <VIcon name="school" />
        <h2 class="erp-title-display-md">Training data</h2>
      </div>
      ...
      <div v-if="!hasPermission || errorMessage || successMessage" class="erp-notices-wrapper" ref="noticeWrapper">
        <VNotice v-if="!hasPermission" type="danger" icon="warning">
          You do not have permissions to update trainings.
        </VNotice>
...
const deleteDialogMessage = computed(() => {
  return 'Are you sure you want to delete this training? This action can not be undone.';
});
...
async function loadItem() {
  ...
  if (!id) {
    errorMessage.value = 'Missing training id.';
    loading.value = false;
    return;
  }
  ...
  try {
    ...
    console.error('Failed to load training', loadError);
...
async function save() {
  ...
  if (!hasPermission.value) {
    errorMessage.value = 'You do not have permissions to update trainings.';
    return;
  }
  ...
  try {
    ...
    successMessage.value = 'Training updated.';
    router.push('/erp/trainings');
  } catch (saveError: any) {
    console.error('Failed to update training', saveError);
...
async function confirmDelete() {
  ...
  try {
    ...
    console.error('Failed to delete training', deleteError);
