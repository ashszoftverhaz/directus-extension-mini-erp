<template>
  <ErpLayout title="ERP - Contracts"
    ...
    <div class="contracts-page">
      <VProgressLinear
        indeterminate
        class="contracts-loading"
        :style="{ visibility: isLoading ? 'visible' : 'hidden' }" />

      <VNotice v-if="isError" type="danger" class="contracts-error">
        Failed to load contracts.
        <VButton @click="refresh">Retry</VButton>
      </VNotice>
      <VNotice v-if="deleteErrorMessage" type="danger" class="contracts-error">
        {{ deleteErrorMessage }}
      </VNotice>
      ...
      <TableComponent
        ...
        :items="contractsWithOtherParty"
        row-key="id"
        :fields="contractFields"
        :column-overrides="contractColumnOverrides"
        selectable
        ...
        :show-empty="!isLoading && contracts.length === 0"
        empty-text="There are no contracts in this collection yet."
        empty-action-label="Create Contract"
        @empty-action="openCreate"
        empty-icon="contract">
        <template #cell-status="{ value }">
          <ContractStatusPill :status="value" />
        </template>

        <template #cell-contract_type="{ value }">
          <span class="value">{{ value.contract_type }}</span>
        </template>
    ...
    <BulkEditDrawer
      ...
      :keys="selectedIds"
      :exclude-fields="['id', 'status', 'contract_id']"
...
import { ContractListItem } from '../../types/contracts';
import { exportContracts, getCollectionInfo } from '../../api/contractsApi';
...
function openCreate() {
  router.push('/erp/contracts/add');
}

function openEdit(id: string | number) {
  router.push(`/erp/contracts/${id}`);
}

const api = useApi();
const isExporting = ref(false);
const exportErrorDialogOpen = ref(false);
const exportErrorMessage = 'Failed to export selected contracts.';
...
async function exportSelectedItems() {
  ...
  try {
    await exportContracts(api, selectedIds.value);
  } catch {
...
const contractColumnOverrides = {
  contract_id: { width: 'minmax(280px, 2fr)', label: 'Contract ID' },
  status: { width: 'minmax(140px, 0.8fr)', sortable: true, resizable: false },
} as const;

const contractsWithOtherParty = computed<(ContractListItem & { other_party: string })[]>(() =>
  contracts.value.map((contract) => {
    let other_party = '';
    if (contract.other_party_type === 'employee') {
      const firstName = contract.employee?.account?.first_name || '';
      const lastName = contract.employee?.account?.last_name || '';
      other_party = [firstName, lastName].filter(Boolean).join(' ');
    } else if (contract.other_party_type === 'partner') {
      other_party = contract.partner?.name || '';
    }

    const result = { ...contract, other_party };
    return result;
  }),
);
...
async function confirmBulkDelete() {
  ...
  try {
    await bulkDeleteItems('contracts', selectedIds.value);
    await refresh();
    selectedIds.value = [];
  } catch {
