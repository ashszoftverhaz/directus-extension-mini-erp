<template>
  <ErpLayout title="ERP - Edit Transaction Category" :getInfo="() => getCollectionInfo(api)">
    <template #headline>
    ...
    <div class="erp-upload-form">
      <div class="erp-form-section-header erp-no-top-margin">
        <VIcon name="category" />
        <h2 class="erp-title-display-md">Transaction category data</h2>
      </div>
      ...
      <div v-if="!hasPermission || errorMessage || successMessage" class="erp-notices-wrapper" ref="noticeWrapper">
        <VNotice v-if="!hasPermission" type="danger" icon="warning">
          You do not have permissions to update transaction categories.
        </VNotice>
...
const deleteDialogMessage = computed(() => {
  return 'Are you sure you want to delete this transaction category? This action can not be undone.';
});
...
async function loadItem() {
  ...
  if (!id) {
    errorMessage.value = 'Missing transaction category id.';
    loading.value = false;
    return;
  }
  ...
  try {
    ...
    console.error('Failed to load transaction category', loadError);
...
async function save() {
  ...
  if (!hasPermission.value) {
    errorMessage.value = 'You do not have permissions to update transaction categories.';
    return;
  }
  ...
  try {
    ...
    successMessage.value = 'Transaction category updated.';
    router.push('/erp/transaction-categories');
  } catch (saveError: any) {
    console.error('Failed to update transaction category', saveError);
...
async function confirmDelete() {
  ...
  try {
    ...
    console.error('Failed to delete transaction category', deleteError);
