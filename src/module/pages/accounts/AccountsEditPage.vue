<template>
  <ErpLayout title="ERP - Edit Account" :getInfo="() => getCollectionInfo(api)">
    <template #headline>
    ...
    <div class="erp-upload-form">
      <div class="erp-form-section-header erp-no-top-margin">
        <VIcon name="account_box" />
        <h2 class="erp-title-display-md">Account data</h2>
      </div>
      ...
      <div v-if="!hasPermission || errorMessage || successMessage" class="erp-notices-wrapper" ref="noticeWrapper">
        <VNotice v-if="!hasPermission" type="danger" icon="warning">
          You do not have permissions to update accounts.
        </VNotice>
...
const deleteDialogMessage = computed(() => {
  return 'Are you sure you want to delete this account? This action can not be undone.';
});
...
async function save() {
  ...
  if (!hasPermission.value) {
    errorMessage.value = 'You do not have permissions to update accounts.';
    return;
  }
  ...
  try {
    ...
    successMessage.value = 'Account updated.';
    router.push('/erp/accounts');
  } catch (saveError: any) {
