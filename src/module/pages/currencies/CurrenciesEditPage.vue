<template>
  <ErpLayout title="ERP - Edit Currency" :getInfo="() => getCollectionInfo(api)">
    <template #headline>
    ...
    <div class="erp-upload-form">
      <div class="erp-form-section-header erp-no-top-margin">
        ...
        <h2 class="erp-title-display-md">Currency data</h2>
      </div>
      ...
      <div v-if="!hasPermission || errorMessage || successMessage" class="erp-notices-wrapper" ref="noticeWrapper">
        <VNotice v-if="!hasPermission" type="danger" icon="warning">
          You do not have permissions to update currencies.
        </VNotice>
...
const deleteDialogMessage = computed(() => {
  return 'Are you sure you want to delete this currency? This action can not be undone.';
});
...
async function loadItem() {
  ...
  if (!id) {
    errorMessage.value = 'Missing currency id.';
    loading.value = false;
    return;
  }
  ...
  try {
    ...
    console.error('Failed to load currency', loadError);
...
async function save() {
  ...
  if (!hasPermission.value) {
    errorMessage.value = 'You do not have permissions to update currencies.';
    return;
  }
  ...
  try {
    ...
    console.error('Failed to update currency', saveError);
...
async function confirmDelete() {
  ...
  try {
    ...
    console.error('Failed to delete currency', deleteError);
