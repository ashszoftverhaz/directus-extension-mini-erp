<template>
  <ErpLayout title="ERP - Edit Contract Type" :getInfo="() => getCollectionInfo(api)">
    <template #headline>
    ...
    <div class="erp-upload-form">
      <div class="erp-form-section-header erp-no-top-margin">
        <VIcon name="contract" />
        <h2 class="erp-title-display-md">Contract type data</h2>
      </div>
      ...
      <div v-if="!hasPermission || errorMessage || successMessage" class="erp-notices-wrapper" ref="noticeWrapper">
        <VNotice v-if="!hasPermission" type="danger" icon="warning">
          You do not have permissions to update contract types.
        </VNotice>
...
import { getCollectionInfo } from '../../api/contractsTypesApi';
...
function goBack() {
  router.push('/erp/contract-types');
}

const deleteDialogMessage = computed(() => {
  return 'Are you sure you want to delete this contract type? This action can not be undone.';
});
...
async function loadItem() {
  ...
  if (!id) {
    errorMessage.value = 'Missing contract type id.';
    loading.value = false;
    return;
  }
  ...
  try {
    ...
    console.error('Failed to load contract type', loadError);
...
async function save() {
  ...
  if (!hasPermission.value) {
    errorMessage.value = 'You do not have permissions to update contract types.';
    return;
  }
  ...
  try {
    ...
    successMessage.value = 'Contract type updated.';
    router.push('/erp/contract-types');
  } catch (saveError: any) {
    console.error('Failed to update contract type', saveError);
...
async function confirmDelete() {
  ...
  try {
    ...
    console.error('Failed to delete contract type', deleteError);
