<template>
  <ErpLayout title="ERP - Add Contract Type" :getInfo="() => getCollectionInfo(api)">
    <template #headline>
    ...
    <div class="erp-upload-form">
      <div class="erp-form-section-header erp-no-top-margin">
        <VIcon name="contract" />
        <h2 class="erp-title-display-md">Add contract type</h2>
      </div>
      ...
      <div v-if="!hasPermission || errorMessage || successMessage" class="erp-notices-wrapper" ref="noticeWrapper">
        <VNotice v-if="!hasPermission" type="danger" icon="warning">
          You do not have permissions to create contract types.
        </VNotice>
...
const breadcrumbs = [
  ...
  { name: 'Add', to: '/erp/contract-types/add', disabled: 'true' },
];
...
const {
  ...
  saveAndCreateNew,
  discardAllChanges,
} = useCreateItem(collection, { successMessage: 'Contract type saved.' });
