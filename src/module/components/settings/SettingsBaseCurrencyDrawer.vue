<template>
  <VDrawer v-model="openModel" direction="right" @cancel="close">
    <template #title>
      <h1 class="drawer-title">Select base currency</h1>
    </template>
    ...
    <div class="drawer-content">
      ...
      <div v-else-if="filteredCurrencies.length === 0" class="drawer-empty-state">
        <EmptyState
...
onMounted(() => {
  if (openModel.value) loadCurrencies();
});

watch(
  () => props.open,
  (isOpen: boolean) => {
    if (isOpen) loadCurrencies();
  },
);
</script>
